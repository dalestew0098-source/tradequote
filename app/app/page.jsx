...
const blankContract = (counters) => ({
  ...
  services: [
    "2× annual tune-up & inspection",
    "Priority scheduling for service calls",
    "10% discount on parts and labour",
...
export default function TradeQuote() {
  ...
  const persist = useCallback((s, d, c, cl) => {
    setSaving(true);
    try {
      window.localStorage?.setItem(KEY, JSON.stringify({ settings: s, docs: d, counters: c, clients: cl }));
  ...
  const newDoc = (type) => {
    if (type === "contract") { setEditing(blankContract(counters)); }
    else { setEditing(blankDoc(type, counters, settings)); }
    setView("edit");
  };
  ...
  const saveDoc = () => {
    ...
    if (exists) { setDocs(docs.map(d => d.id === editing.id ? editing : d)); }
      ...
      setDocs([editing, ...docs]);
      setCounters({ ...counters, [editing.type]: (counters[editing.type] || 0) + 1 });
    }
  ...
  const deleteDoc = (id) => {
    ...
    setDocs(docs.filter(d => d.id !== id)); setView("list");
  };
  ...
  const addItem = () => setEditing({
    ...editing,
    items: [...editing.items, { id: "i" + Date.now(), desc: "", qty: 1, cost: "", markup: settings.defaultMarkup || 30, isMaterial: false }],
  });
    ...
    <div style={wrap}>
      ...
      {view === "settings" && (
        <div style={pad}>
          ...
          {[
            ["bizName", "Business name", "e.g. Arctic Air HVAC"],
          ...
          <Field label="Default parts markup %">
            <input value={settings.defaultMarkup} inputMode="decimal"
              onChange={e => setSettings({ ...settings, defaultMarkup: e.target.value.replace(/[^0-9.]/g, "") })}
              style={input} placeholder="e.g. 30" />
            <div style={{ fontSize: 11, color: "#9CA3AF", marginTop: 4 }}>
              Applied automatically to materials. Labour is always 0%.
            </div>
          </Field>
                  ...
                  <div key={d.id} className="row" style={listRow} onClick={() => editDoc(d)}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 12, color: "#9CA3AF" }}>
                        {d.type === "contract" ? "Service Contract" : d.type === "invoice" ? "Invoice" : "Quote"} · {d.date}
                      </div>
                    </div>
                    <div style={{ fontWeight: 700, fontSize: 16 }}>
                      {d.type === "contract" ? (d.price ? money(d.price) + "/" + (d.billingCycle === "annual" ? "yr" : "mo") : "—") : money(total)}
                    </div>
                  </div>
                );
              })}
      ...
      {view === "edit" && editing && (
        <div style={pad}>
          ...
          <h2 style={pageTitle}>
            {isContract ? "🔧 Service Contract" : editing.type === "quote" ? "Quote" : "Invoice"} for…
          </h2>
          ...
          {isContract && (
            <>
              ...
              <Field label="Equipment covered">
                <input value={editing.equipment} onChange={e => setEditing({ ...editing, equipment: e.target.value })} style={input} placeholder="Make, model, serial numbers" />
              </Field>
              <div style={{ display: "flex", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                <Field label="Price" style={{ flex: 1 }}>
                  <div style={{ position: "relative" }}>
                    ...
                    <input value={editing.price} inputMode="decimal" placeholder="0.00"
                ...
                <Field label="Billing" style={{ flex: 1 }}>
                  <select value={editing.billingCycle} onChange={e => setEditing({ ...editing, billingCycle: e.target.value })}
              ...
              {editing.services.map((s, i) => (
                <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                  <input value={s} onChange={e => {
                    const services = editing.services.map((sv, si) => si === i ? e.target.value : sv);
                    setEditing({ ...editing, services });
                  }} style={{ ...input, flex: 1 }} />
                  <button onClick={() => setEditing({ ...editing, services: editing.services.filter((_, si) => si !== i) })}
                    style={delBtn}>×</button>
                </div>
              ))}
              <button onClick={() => setEditing({ ...editing, services: [...editing.services, ""] })}
                style={{ ...outlineBtn, borderColor: "#D1D5DB", color: "#374151", width: "100%", marginBottom: 16 }}>
                + Add service
              </button>
            </>
          )}
          ...
          {!isContract && (
            <>
              ...
              <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 6, marginBottom: 14 }}>
                {HVAC_TEMPLATES.map(tpl => (
                  <button key={tpl.label} onClick={() => applyTemplate(tpl)}
                    style={{ ...chip, flexShrink: 0, whiteSpace: "nowrap", background: "#FFF7ED", borderColor: accent, color: accent }}>
                    {tpl.label}
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                ...
                <button onClick={() => setMarkupMode(m => !m)}
                  style={{ ...chip, fontSize: 11, padding: "4px 10px", background: markupMode ? accent + "15" : "#fff", color: markupMode ? accent : "#6B7280", borderColor: markupMode ? accent : "#D1D5DB" }}>
                  {markupMode ? "✓ Markup mode ON" : "Markup mode"}
                </button>
              </div>
              ...
              {markupMode && (
                <div style={{ background: "#FFF7ED", border: "1px solid #FED7AA", borderRadius: 10, padding: "10px 12px", marginBottom: 12, fontSize: 12, color: "#92400E" }}>
                  <strong>Markup mode:</strong> Enter your <em>cost price</em> for materials and a markup %. Labour rows use rate directly.
                </div>
              )}
              ...
              {editing.items.map((it, i) => (
                <div key={it.id} style={itemCard}>
                  <div style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
                    <input value={it.desc} placeholder="Description"
                      onChange={e => updItem(i, "desc", e.target.value)} style={{ ...input, flex: 1 }} />
                    {markupMode && (
                      <button onClick={() => updItem(i, "isMaterial", !it.isMaterial)}
                        style={{ ...chip, fontSize: 11, padding: "6px 10px", flexShrink: 0,
                          background: it.isMaterial ? "#DBEAFE" : "#F3F4F6",
                          color: it.isMaterial ? "#1D4ED8" : "#6B7280",
                          borderColor: it.isMaterial ? "#93C5FD" : "#E5E7EB" }}>
                        {it.isMaterial ? "Part" : "Labour"}
                      </button>
                    )}
                  </div>
                    ...
                    {markupMode && it.isMaterial ? (
                      <>
                        ...
                        <div style={{ position: "relative", width: 64 }}>
                          ...
                          <span style={{ position: "absolute", right: 7, top: "50%", transform: "translateY(-50%)", color: "#9CA3AF", fontSize: 12 }}>%</span>
                        </div>
                        <span style={{ fontSize: 12, color: "#047857", fontWeight: 700, minWidth: 60, textAlign: "right" }}>
              ...
              <div style={totalsBox}>
                {(() => {
                  ...
                  const materialCost = editing.items.reduce((a, it) =>
                    a + (it.isMaterial ? (Number(it.cost) || 0) * (Number(it.qty) || 0) : 0), 0);
                  const materialSell = editing.items.reduce((a, it) =>
                    a + (it.isMaterial ? (Number(it.qty) || 0) * itemRate(it) : 0), 0);
                  const profit = materialSell - materialCost;
                  return (
                    <>
                      ...
                      {markupMode && profit > 0 && (
                        <div style={{ marginTop: 10, background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: "#166534" }}>
                          💰 Parts margin: {money(profit)} ({materialCost > 0 ? Math.round((profit / materialCost) * 100) : 0}% on cost)
                        </div>
                      )}
                    </>
                  );
          ...
          <Field label="Notes" style={{ marginTop: 16 }}>
            <textarea value={editing.notes} rows={2} placeholder="Anything else for the client…"
  ...
  return (
    <div className="sheet" style={sheet}>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
        {[{ label: "Service Provider", name: settings.bizName, phone: settings.phone, email: settings.email },
          { label: "Customer", name: doc.clientName, phone: doc.clientEmail, email: doc.clientAddress }
        ].map(p => (
      ...
      <div style={{ background: "#FFF7ED", border: `1px solid ${accent}40`, borderRadius: 10, padding: "14px 16px", marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 13, color: "#374151", marginTop: 6 }}>
          <strong>{doc.price ? money(doc.price) : "—"}</strong> billed {doc.billingCycle === "annual" ? "annually" : "monthly"}
        </div>
      </div>
      {doc.equipment && (
        <div style={{ marginBottom: 20 }}>
          <div style={noteLabel}>Equipment covered</div>
          <div style={{ fontSize: 13, color: "#374151" }}>{doc.equipment}</div>
        </div>
      )}
      <div style={{ marginBottom: 24 }}>
        <div style={noteLabel}>Services included</div>
        {doc.services.filter(Boolean).map((s, i) => (
      ...
      <div style={{ display: "flex", gap: 24, marginTop: 40 }}>
        {[settings.bizName || "Service Provider", doc.clientName || "Customer"].map(name => (
          <div key={name} style={{ flex: 1 }}>
            <div style={{ borderTop: "1.5px solid #374151", paddingTop: 6 }}>
              <div style={{ fontSize: 11, color: "#6B7280" }}>{name}</div>
              <div style={{ fontSize: 11, color: "#9CA3AF", marginTop: 2 }}>Signature & Date</div>
            </div>
          </div>
        ))}
      </div>
...
const logoDot = { width: 36, height: 36, borderRadius: 9, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontFamily: brand, fontSize: 18 };
const pad = { padding: "18px" };
const pageTitle = { fontFamily: brand, fontWeight: 800, fontSize: 22, margin: "0 0 4px" };
